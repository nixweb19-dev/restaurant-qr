...
async function initAdmin() {
    ...
    // 5. Menü Yönetimini Başlat
    await initMenuManagement();
}
...
async function initMenuManagement() {
    ...
    productForm.addEventListener('submit', async (e) => {
        ...
        if (!catId) {
            alert("Lütfen bir kategori seçin!"); return;
        }
        ...
        btn.disabled = false; btn.innerText = 'Ürünü Kaydet';
...
async function fetchMenuData() {
    ...
    renderMenuManagement(categories, products);
}

function renderMenuManagement(categories, products) {
    if (!productCategorySelect || !menuPreviewList) return;

    // Select kutusunu güncelle
    productCategorySelect.innerHTML = '<option value="">Kategori Seçin</option>';
    categories.forEach(cat => {
        productCategorySelect.innerHTML += `<option value="${cat.id}">${escapeHTML(cat.name)}</option>`;
    });

    // Önizleme listesini güncelle
    if (categories.length === 0) {
        menuPreviewList.innerHTML = '<p style="color: var(--text-muted); text-align: center; margin-top: 2rem;">Henüz kategori eklemediniz.</p>';
        return;
    }

    let html = '';
    categories.forEach(cat => {
        html += `<div style="margin-bottom: 1.5rem; background: #f8fafc; padding: 1rem; border-radius: 8px; border: 1px solid var(--border);">`;
        html += `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid var(--primary); padding-bottom: 0.5rem; margin-bottom: 1rem;">
                    <h4 style="font-size: 1.1rem; color: var(--text-main);">${escapeHTML(cat.name)}</h4>
                    <button onclick="deleteCategory('${cat.id}')" class="btn btn-sm" style="color:#ef4444; background:none; border:none; padding:0;"><i class="fa-solid fa-trash"></i></button>
                 </div>`;
        
        // Bu kategoriye ait ürünler
        const catProducts = products.filter(p => p.category_id === cat.id);
        if (catProducts.length === 0) {
            html += `<p style="font-size: 0.9rem; color: var(--text-muted);">Bu kategoride henüz ürün yok.</p>`;
        } else {
            catProducts.forEach(prod => {
                html += `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; border-bottom: 1px dashed var(--border);">
                        <div>
                            <strong style="display:block;">${escapeHTML(prod.name)}</strong>
                            <span style="font-size: 0.85rem; color: var(--text-muted);">${escapeHTML(prod.description || '')}</span>
                        </div>
                        <div style="display:flex; align-items:center; gap: 10px;">
                            <strong style="color: var(--primary);">${prod.price} TL</strong>
                            <button onclick="deleteProduct('${prod.id}')" style="color:#ef4444; background:none; border:none; cursor:pointer;"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>
                `;
            });
    ...
    menuPreviewList.innerHTML = html;
}

window.deleteCategory = async (id) => {
    if (confirm("Bu kategoriyi silmek istediğinize emin misiniz? (İçindeki ürünler de silinir!)")) {
